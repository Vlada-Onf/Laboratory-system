...
const handleAddComponentSubmit = useCallback(async (formData, file) => {
  console.log('📋 ADD Table отримав:', { 
    name: formData.name, 
    file: file?.name,
...
}, [addComponent, closeEditModal, fetchComponents]);

const handleComponentSubmit = useCallback(async (formData, file) => {
  console.log('📋 UPDATE Table отримав:', { 
    name: formData.name, 
    file: file?.name 
  });
  ...
  try {
    ...
    await updateComponent(componentId, formData, file);
    closeEditModal();
    await fetchComponents();
  } catch (error) {
    console.error('Помилка оновлення:', error);
  }
}, [editModal.component, updateComponent, closeEditModal, fetchComponents]);
