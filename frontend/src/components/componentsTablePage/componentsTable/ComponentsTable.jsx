...
const ComponentsTable = ({ onAddNeed }) => {
  ...
  const { 
    ...
    editModal, openEditModal, closeEditModal, fetchComponents 
  } = useComponentsStore();

  const isEditing = Boolean(editModal.component);

  const columns = useMemo(() => [
    { field: 'component', headerName: 'Компонент', flex: 2, minWidth: 250, sortable: false,
      renderCell: ({ row }) => (
        <ClickableComponentCell
          image={row.photoUrl}
          name={row.name}
          id={row.id}
          onClick={(id) => window.location.href = `/front-components/${id}`}
        />
      )
    },
    ...
    { field: 'documentationLink', headerName: 'Документація', flex: 1.5, minWidth: 180,
      renderCell: ({ value }) => <LinkBadge url={value} color="#08273b" />
    },
    { field: 'quantity', headerName: 'К-сть', flex: 0.8, minWidth: 80,
      renderCell: ({ value }) => <Typography fontWeight={600}>{value} шт</Typography>
    },
    { field: 'price', headerName: 'Ціна', flex: 1, minWidth: 100,
      renderCell: ({ value }) => <Typography fontWeight={600}>{value ? `${value} ₴` : '—'}</Typography>
    },
    { field: 'tags', headerName: 'Теги', flex: 1.5, minWidth: 150,
      renderCell: ({ row }) => <TagsCell value={row.tags} />
    },
    { field: 'rowActions', headerName: '', width: 80, sortable: false, filterable: false,
      renderCell: ({ row }) => (
        <ButtonsCell
          row={row}
  ...
  const handleAddNeed = useCallback((formData) => {
    if (!selectedRow || !onAddNeed) return;
